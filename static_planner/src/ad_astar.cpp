...
{
        ROS_INFO("try to search path in planner, heading: %f", start_th);
        ROS_INFO("start in map: %f, %f; end in map: %f, %f", start_x, start_y, end_x, end_y);
	...
	wrapper_.initialize(costs, toIndex(start_x_up, start_y_up), lethal_cost_, nx_, ny_, rough_lenth_, resolution_, unknown_);
        ROS_INFO("initialization of wrapper finish");
        wrapper_.printNewCosts();
        wrapper_.printOriCosts();
        ROS_INFO("lowerleft of new costmap: %d", wrapper_.getLowerLeft());
        ROS_INFO("lowerleft of new costmap index: %d, %d", wrapper_.getLowerLeftX(), wrapper_.getLowerLeftY());
        ROS_INFO("rough len of new cosmtap: %d, %f, %f", wrapper_.getRoughLen(), wrapper_.getRoughLenInWorld(), wrapper_.getOriMapResolution());
        ROS_INFO("size of new map: %d, %d", wrapper_.getNewMapWidth(), wrapper_.getNewMapHeight());
	...
	double start_y_wrapper = start_wrapper.second;
        ROS_INFO("start in new costmap: %f, %f", start_x_wrapper, start_y_wrapper);
	...
	double end_y_wrapper = end_wrapper.second;
        ROS_INFO("end in new costmap: %f, %f", end_x_wrapper, end_y_wrapper);
	...
	if(searchPath(new_costs, start_x_wrapper, start_y_wrapper, start_th, end_x_wrapper, end_y_wrapper, end_th, wrapper_.getNewMapWidth() * wrapper_.getNewMapHeight() * 2)){
	    ...
	    while(temp->p_ != nullptr){
	    	...
	    	plan.push_back(std::pair<float, float>((float)x, (float)y));
               ...
               std::string filename = "/home/wz9562/Documents/path.txt";
               std::fstream fs(filename, std::ios_base::app);
               if(!fs.is_open()){
                    ROS_INFO("wrong to open file to record path");
               } 
               else{
                    fs << x << "," << y << " ";
               }
            ...
            ROS_INFO("length of path is: %d", plan.size());
	    return true;
	}
	else
		return false;
...
bool AdAStarPlanner::searchPath(int* costs, double start_x, double start_y, double start_th, double end_x, double end_y, double end_th, int cycles){
    ...
    ROS_INFO("check if next pos is reachable: %d, %d", (int)start_x + next_pos.first, (int)start_y + next_pos.second);
	if(costs[start_i] == 1){
                ROS_INFO("next pos is unreachable");
		return false;
        }
	
    // ROS_INFO("prepare to start search");
	queue_.push_back(Index(start_i, 0, start_th_f));

    // ROS_INFO("current index is: %d", start_i);
    ...
    while (queue_.size() > 0 && cycle < cycles) {
        ...
        if (i == goal_i){
			end_index_ = top;
            ROS_INFO("finish searching for path, successfully");
            return true;
		}
	...
	// go through all neighbor of Index with minimum cost
        // add(costs, top, i, i + 1, (int)end_x, (int)end_y);
        // add(costs, top, i, i - 1, (int)end_x, (int)end_y);
        // add(costs, top, i, i + new_nx_, (int)end_x, (int)end_y);
        // add(costs, top, i, i - new_nx_, (int)end_x, (int)end_y);
        // add(costs, top, i, i + new_nx_ - 1, (int)end_x, (int)end_y);
        // add(costs, top, i, i + new_nx_ + 1, (int)end_x, (int)end_y);
        // add(costs, top, i, i - new_nx_ - 1, (int)end_x, (int)end_y);
    ...
    ROS_INFO("finish searching for path, failed");
    return false;
}
...
void AdAStarPlanner::add(int* costs, Index min_fn_ind, int i, int next_i, int end_x,int end_y) {
    ...
    if (next_i < 0 || next_i >= wrapper_.getNewMapSize()){
        // ROS_INFO("neighbor is out of bounds");
        return;
    }
    ...
    // ROS_INFO("calc index orientation");
	...
	float distance = hypot(end_x - x, end_y - y); // euclidean distance
        ...
        // ROS_INFO("find neighbor in open list");
